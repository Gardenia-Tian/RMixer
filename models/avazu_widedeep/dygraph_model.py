...
# import net
import models.avazu_widedeep.net as net
...
class DygraphModel():
    ...
    def create_feeds(self, batch_data, config):
        ...
        for b in batch_data:
            sparse_tensor.append(paddle.to_tensor(b.numpy().astype('int64').reshape(-1, 1)))
            dense_tensor.append(paddle.to_tensor(b.numpy().astype('float32').reshape(-1, 1)))
        label = sparse_tensor[-1]
        # dense_tensor = paddle.to_tensor(batch_data[-1].numpy().astype('float32').reshape(-1, dense_feature_dim))  
        dense_tensor = paddle.stack(dense_tensor[:-1])
        dense_tensor = paddle.reshape(dense_tensor, [-1, dense_feature_dim])
        # print(len(sparse_tensor))
        # print(len(sparse_tensor[0]))

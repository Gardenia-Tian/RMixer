...
class DCN_V2Layer(nn.Layer):
    def __init__(self, sparse_feature_number, sparse_feature_dim,
        ...
        self.dense_emb = nn.Linear(self.dense_feature_dim, (self.sparse_feature_dim * self.dense_feature_dim))
    ...
    def forward(self, sparse_inputs, dense_inputs):
        ...
        # EmbeddingLayer
        sparse_inputs_concat = paddle.concat(sparse_inputs, axis=1)  #Tensor(shape=[bs, 26])
            
        sparse_embeddings = self.embedding(sparse_inputs_concat)  # shape=[bs, 26, dim]
        # print("sparse_embeddings shape:",sparse_embeddings.shape)

        sparse_embeddings_re = paddle.reshape(sparse_embeddings, shape=[-1, self.sparse_num_field * self.sparse_feature_dim])
            
        dense_embeddings = self.dense_emb(dense_inputs)  # # shape=[bs, 13, dim]
            

        feat_embeddings = paddle.concat([sparse_embeddings_re, dense_embeddings], 1)

        ...
        for i, unit in enumerate(units_list[:-1]):
            if i != len(units_list) - 1:
                dense = paddle.nn.Linear(
                    ...
                    weight_attr=paddle.ParamAttr(initializer=paddle.nn.initializer.TruncatedNormal(std=1.0 / math.sqrt(unit))))    
    ...
    def __init__(self, sparse_field_num, sparse_feature_num,
        ...
        self.sparse_weight = paddle.nn.Embedding(
            ...
            weight_attr=paddle.ParamAttr(
                ...
                initializer=paddle.nn.initializer.Uniform()))
        self.dense_linear = paddle.nn.Linear(in_features=self.dense_feature_dim, out_features=1)
